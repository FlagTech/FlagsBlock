...
Blockly.Blocks['procedures_defreturn'] = {
  ...
  getReturnType: function() {
    ...
    if (returnBlock) {
      ...
      // modified by Yu-Hsiung Chiu
      if (isVarGetBlock(returnBlock)) {
        returnType = getVarBlockType(returnBlock)
      }
      else {
...
Blockly.Blocks['procedures_callreturn'] = {
  ...
  getBlockType: function() {
    var type = Blockly.Types.NUMBER
    var name = this.getFieldValue('NAME');
    var blocks = this.workspace.getAllBlocks(); 

    for (var i = 0; i < blocks.length; i++) {
      if (blocks[i].getProcedureDef && blocks[i].getFieldValue('NAME') == name) {
        type = blocks[i].getReturnType();
        break;
      }
    } 

...
Ardublockly.largeIdeButtonSpinner = function(active) {
  ...
  var progressBarContainer = document.getElementById('ide_output_progress_bar_container');
  var progressBar = document.getElementById('ide_output_progress_bar');
...
Ardublockly.arduinoIdeOutput = function(bodyEl) {
  ...
  //collapseIdeOutputTimer = setTimeout(function(){ collapseIdeOutput(); }, 5000);
};
...
Ardublockly.sketchNameSizeEffect = function() {
  ...
  var correctInput = function() {
    ...
    if ($(this).val() == '') {
      ...
      $(this).val(Ardublockly.getLocalStr('projectName'));
      $(this).attr('size', $(this).val().length);
    }

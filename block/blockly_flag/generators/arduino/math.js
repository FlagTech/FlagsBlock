...
Blockly.Arduino['math_change2'] = function(block) {
  var argument0 = Blockly.Arduino.valueToCode(block, 'DELTA',
      Blockly.Arduino.ORDER_ADDITIVE) || '0';
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
...
Blockly.Arduino['math_random_float'] = function(block) {
  return ['((float)rand() / (float)RAND_MAX)', Blockly.Arduino.ORDER_UNARY_POSTFIX];
};

Blockly.Arduino['math_statistics'] = function(block) {
  var code;
  var numbers = [];
  var operator = block.getFieldValue('OP');
  var warningTextName = "math_statistics_itemCount";

  if (block.itemCount_ < 2) {
    this.setWarningText(Blockly.Msg.MATH_CALCULATE_WARNING, warningTextName);
    return [0, Blockly.Arduino.ORDER_UNARY_POSTFIX];
  }

  this.setWarningText(null, warningTextName);
  for (var n = 0; n < block.itemCount_; n++) {
    numbers[n] = Blockly.Arduino.valueToCode(block, 'ADD' + n, 
      Blockly.Arduino.ORDER_NONE) || '0';
  }

  switch (operator) {
    case "AVERAGE":
        code = '(' + numbers.join(' + ') + ') / ' + block.itemCount_;
        break;
    case "SUM":
        code = '(' + numbers.join(' + ') + ')';
        break;
    case "MAX":
        ...
        code = 'max(' + numbers.join(', max(') + ', ' + lastNum + Array(block.itemCount_).join(")");
        break;
    case "MIN":
        var lastNum = numbers.pop();
        code = 'min(' + numbers.join(', min(') + ', ' + lastNum + Array(block.itemCount_).join(")");
        break;
    default: 
        code = '0';
  }

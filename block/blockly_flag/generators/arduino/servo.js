...
Blockly.Arduino['servo_use'] = function (block) {
    var pin = block.getFieldValue('PIN');

    Blockly.Arduino.reservePin(
        block, pin, Blockly.Arduino.PinTypes.SERVO, 'Servo Write');

    var servoName = block.getFieldValue('OBJNAME');
    ...
    var setupCode = servoId + '.attach(' + pin + ');';
    Blockly.Arduino.addSetup(servoId, setupCode, true);

    var code = '';
    return code;
};

Blockly.Arduino['servo_put'] = function (block) {
    var servoName = block.getFieldValue('OBJNAME');
    var servoId = Blockly.Arduino.variableDB_.getName(
        servoName,
        Blockly.Variables.NAME_TYPE/*blocklyArray_NAME_TYPE*/);
...
Blockly.Arduino['servo_get'] = function (block) {
    var servoName = block.getFieldValue('OBJNAME');
    var servoId = Blockly.Arduino.variableDB_.getName(
        servoName,
        Blockly.Variables.NAME_TYPE/*blocklyArray_NAME_TYPE*/);
...
Blockly.Arduino['servo_write'] = function (block) {
    var pinKey = Blockly.Arduino.valueToCode(
        block, 'SERVO_PIN', Blockly.Arduino.ORDER_ATOMIC);
    var servoAngle = Blockly.Arduino.valueToCode(
        block, 'SERVO_ANGLE', Blockly.Arduino.ORDER_ATOMIC) || '90';
    ...
    //Blockly.Arduino.addSetupBottom('servo_' + pinKey, setupCode, true);

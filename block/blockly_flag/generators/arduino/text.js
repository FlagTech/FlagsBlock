...
// added by Mee
Blockly.Arduino['text_atoi'] = function(block) {
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var code = varName + '.toInt()';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_atof'] = function(block) {
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var code = varName + '.toFloat()';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_lf'] = function(block) {
  return ['"\\x0A"', Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_cr'] = function(block) {
  return ['"\\x0D"', Blockly.Arduino.ORDER_UNARY_ATOMIC];
};
...
Blockly.Arduino['text_format'] = function(block) {
  ...
  var width = Blockly.Arduino.valueToCode(
    block, 'WIDTH', Blockly.Arduino.ORDER_ATOMIC) || '0';
  ...
  func.push('    content = " " + content;');
  func.push('  return content;');
  func.push('}');
  var funcName = Blockly.Arduino.addFunction(
      '__paddingStr', func.join('\n'));

  var code = '__paddingStr(String(' + content + '), ' + width + ')';
  return [code, Blockly.Arduino.ORDER_ATOMIC];
};

Blockly.Arduino['text_char'] = function(block) {
  var charCode = Blockly.Arduino.valueToCode(
    block, 'CHARCODE', Blockly.Arduino.ORDER_ATOMIC) || '0';

  var code = 'String(char(' + charCode + '))';
  return [code, Blockly.Arduino.ORDER_ATOMIC];
};
...
Blockly.Arduino['text_substring'] = function(block) {
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var fromPos = Blockly.Arduino.valueToCode(
    block, 'FROM', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var endPos = Blockly.Arduino.valueToCode(
    block, 'TO', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var code = varName + '.substring(' + fromPos + ', ' + endPos + ')';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_substring_end'] = function(block) {
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var fromPos = Blockly.Arduino.valueToCode(
    block, 'FROM', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var code = varName + '.substring(' + fromPos + ')';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_char_code'] = function(block) {
  var txt = Blockly.Arduino.valueToCode(
    block, 'TEXT', Blockly.Arduino.ORDER_ATOMIC) || '';
  var idx = Blockly.Arduino.valueToCode(
    block, 'INDEX', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var code = 'String(' + txt + ')[' + idx + ']';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

Blockly.Arduino['text_find'] = function(block) {
  var varName = Blockly.Arduino.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var fromPos = Blockly.Arduino.valueToCode(
    block, 'FROM', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var substr = Blockly.Arduino.valueToCode(
    block, 'SUBSTRING', Blockly.Arduino.ORDER_ATOMIC) || '""';
  var code = varName + '.indexOf(' + substr + ', ' + fromPos + ')';
  return [code, Blockly.Arduino.ORDER_UNARY_ATOMIC];
};

...
Blockly.Arduino.init = function(workspace) {
  ...
  // Create a dictionary of setups to be printed in the end of the setup function
  Blockly.Arduino.setupsBottom_ = Object.create(null);
  ...
  //   if (varsWithTypes[varName] === Blockly.Types.ARRAY) {
  //     delete varsWithTypes[varName];
  //   }
  // }
...
Blockly.Arduino.finish = function(code) {
  ...
  for (var name in Blockly.Arduino.setupsBottom_) {
    setups.push(Blockly.Arduino.setupsBottom_[name]);
  }
  ...
  // var loop = 'void loop() {\n  ' + code.replace(/\n/g, '\n  ') + '\n}';
  var setup = '// setup() 會先被執行且只會執行一次\nvoid setup() {' + setups.join('\n  ') + '\n}\n\n';
  var loop = '// loop() 裡面的程式會不斷重複執行\nvoid loop() {\n  ' + code.replace(/\n/g, '\n  ') + '\n}';
...
Blockly.Arduino.addSetupBottom = function(setupTag, code, overwrite) {
  var overwritten = false;
  if (overwrite || (Blockly.Arduino.setupsBottom_[setupTag] === undefined)) {
    Blockly.Arduino.setupsBottom_[setupTag] = code;
    overwritten = true;
  }
  return overwritten;
};
...
Blockly.Arduino.quote_ = function(string) {
  ...
  string = string.replace(/\\/g, '\\\\')
                 ...
                 .replace(/'/g, '\\\'');
  // return '\"' + string + '\"';
  return 'u8\"' + string + '\"'; // modified by Mee to use UTF-8 string
};
...
Blockly.Arduino.scrub_ = function(block, code) {
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    for (var x = 0; x < block.inputList.length; x++) {
      if (block.inputList[x].type == Blockly.INPUT_VALUE) {
        ...
        if (childBlock) {
          if (childBlock.prefixCode) {
            prefixCode += childBlock.prefixCode;
          }
  ...
  var highlight_prefix = '';
  var highlight_postfix = '';


  // 如果 code 是數字積木的時候, 不要 high light
  // 因為其他積木產生程式碼時需要以數字或字串來判斷是否為變數
  if (isSelectedBlockHighlight &&
      block == Blockly.selected &&
      block.type != "arduino_loop" &&
      block.type != "math_number" ){
    highlight_prefix = selectedBlockHighlightTextPrefix;
    highlight_postfix = selectedBlockHighlightTextPostfix;

    if ( typeof code === 'string' &&  code.slice(-1) == '\n') {
      code = code.slice(0, -1) + highlight_postfix + '\n';
    }
    else {
      code = code + highlight_postfix;
    }
  }

  return highlight_prefix + commentCode + prefixCode + code + nextCode;
};

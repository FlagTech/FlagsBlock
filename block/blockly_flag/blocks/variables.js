...
Blockly.Blocks['variables_set'] = {
   ...
   */
  init: function() {
    //var varName = Blockly.Variables.generateUniqueName(Blockly.mainWorkspace);
    this.jsonInit({
...
Blockly.Blocks['variables_init'] = {
  ...
  // added by Yu-Hsiung Chiu
  onchange: function(e) {

    // 若有兩個同樣變數的設定初值積木, 不知為何會一直重複產生 warningOpen 事件
    // 而進入 onchange, 無窮迴圈會導致系統變慢或者當掉
    if (e.element === "warningOpen") throw "exit";
    // 以下寫法無效
    // if (e.element === "warningOpen") return false;

    // 如果沒有放在 setup 裡面就顯示警告訊息
    var inSETUP = false;
    var block = this;
    do {
    ...
    if (inSETUP) {
      this.setWarningText(null, 'inSETUP');
    } else {
      this.setWarningText(Blockly.Msg.VARIABLES_INIT_NOT_IN_SETUP_WARNING, 'inSETUP');
    }

    // 如果一個變數有兩個設定初值積木就顯示警告訊息
    var isRedefined = false;
    block = this.getPrevBlock();
    while (block) {
    ...
    if (isRedefined) {
      this.setWarningText(
        Blockly.Msg.VARIABLES_INIT_REDEFINED_WARNING.replace("%1", 
          this.getFieldValue('VAR')), 
        'isRedefined');
    } else {
      this.setWarningText(null, 'isRedefined');
    }

...
def get_serial_ports():
    ...
    if not ports:
        json_data['options'].append({
            'value': 'no_ports',
            'display_text': '找不到可用的序列埠'})
        json_data.update({'selected': 'no_ports'})
    else:
        for key in ports:
            # 忽略 [port_last]
            if key == ServerCompilerSettings().get_serial_port_key_last():
                continue
            json_data['options'].append(
...
# Ref: http://stackoverflow.com/questions/2885190/using-pythons-configparser-to-read-a-file-without-section-name
def read_properties(filename):
    ...
    with open(filename, "rb") as csvfile:
        reader = csv.reader(csvfile, delimiter=str('='), quoting=csv.QUOTE_NONE)
        for row in reader:
            if len(row) != 2:
...
def write_properties(filename,dictionary):
    ...
    with open(filename, "wb") as csvfile:
        writer = csv.writer(csvfile, delimiter=str('='), quoting=csv.QUOTE_NONE)

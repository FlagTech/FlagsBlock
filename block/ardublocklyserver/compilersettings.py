...
class ServerCompilerSettings(object):
    ...
    # Modified by Mee:hiding boards except UNO and NANO from setting menu.
    __arduino_types = {'Uno': 'arduino:avr:uno',
                       'Nano': 'arduino:avr:nano:cpu=atmega328old'#":cpu=atmega328" added by Mee to specify cpu,
                    ...
                    #  ,'WeMos D1 mini': 'esp8266com:esp8266:d1_mini:CpuFrequency=160,FlashSize=4M1M,LwIPVariant=v2mss536,Debug=Disabled,DebugLevel=None____,UploadSpeed=115200'
                      ,'WeMos D1 mini': 'esp8266com:esp8266:d1_mini:xtal=80,vt=flash,exception=disabled,eesz=4M,ip=lm2f,dbg=Disabled,lvl=None____,wipe=none,baud=115200'
                      , 'NodeMCU': 'esp8266com:esp8266:nodemcuv2:xtal=80,vt=flash,exception=disabled,eesz=4M,ip=lm2f,dbg=Disabled,lvl=None____,wipe=none,baud=115200'
                    }
    ...
    def __initialise(self, settings_dir=None):
        ...
        self.__serial_port_key_last = 'port_last'  # 保留上次的設定值
        self.__block_root_dir = None
        if settings_dir:
        ...
        self.__block_root_dir = os.path.dirname(self.__settings_path)
    ...
    def get_serial_port_key_last(self):
        return self.__serial_port_key_last
    ...
    def set_compiler_dir_default(self):
        #self.__compiler_dir = None
        # Modified by Mee 2019/4/15
        # Added compatibility for macOS and Linux
        if sys.platform == 'darwin':          # macOS
            self.__compiler_dir = os.path.join(
                os.path.dirname(self.__block_root_dir), "Arduino.app/Contents/MacOS/", "Arduino") # modified by mee
        elif sys.platform.find('linux') == 0: # Linux
            self.__compiler_dir = os.path.join(
                os.path.dirname(self.__block_root_dir), "arduino", "arduino") # modified by mee
        else:                                 # Windows
            self.__compiler_dir = os.path.join(
                os.path.dirname(self.__block_root_dir), "arduino", "arduino_debug.exe") # modified by mee
    ...
    def set_sketch_name_default(self):
         ...
         self.__sketch_name = 'FlagsBlockSketch'
    ...
    def set_serial_port(self, new_port):
        ...
        """
        if self.__serial_port_key_last in self.__serial_ports:
            del self.__serial_ports[self.__serial_port_key_last]
    ...
    def set_serial_port_default(self):
        ...
        if not self.__serial_ports:
            ...
            self.__serial_port_key = sorted(self.__serial_ports.keys())[-1]
            self.__serial_port_value = \
    ...
    def set_serial_port_from_file(self, new_port_value):
        ...
        if set_default:
            ...
            if new_port_value == 'None':
                self.set_serial_port_default()
                print('Default Serial Port set:\n\t%s' % self.__serial_port_value)
            else:
                self.__serial_port_key = self.__serial_port_key_last
                self.__serial_port_value = new_port_value
                self.__serial_ports[self.__serial_port_key] = self.__serial_port_value
                print('Using last Serial Port:\n\t%s' % self.__serial_port_value)
    ...
    def populate_serial_port_list(self):
        ...
        available.
        """
        last_serial_port_value = None  # 保留上次的設定值
        if self.__serial_port_key_last in self.__serial_ports:
            last_serial_port_value = self.__serial_ports[self.__serial_port_key_last]
        ...
        if port_list:
            ...
            for item in port_list:
                ...
                if item == last_serial_port_value:
                    self.__serial_port_key = id_string
                    self.__serial_port_value = item
                    last_serial_port_value = None
                port_id += 1

        if last_serial_port_value:
            self.__serial_ports[self.__serial_port_key_last] = last_serial_port_value
    ...
    def read_settings(self):
        ...
        if os.path.exists(self.get_firstrun_path()):
            print('\nFound first run file. Resetting all configurations')
            settings_dict = None
        ...
        if settings_dict:
            ...
            print('\nArduino preferences will be set to the default values.')
            import shutil
            sketch_full_dir = os.path.join(self.__sketch_dir, self.__sketch_name)
            try:
                shutil.copy2(
                    os.path.join(sketch_full_dir, 'preferences_bak.txt'), 
                    os.path.join(sketch_full_dir, 'preferences.txt') )
    ...
    def get_firstrun_path(self):
        return os.path.join(self.__block_root_dir, "FIRSTRUN")

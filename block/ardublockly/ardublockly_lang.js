...
Ardublockly.initLanguage = function() {
  ...
  // 因為 index.html 裡面很多 text 是英文, 所以要強迫更新
  Ardublockly.duplicateDefaultLang();
  Ardublockly.injectLanguageJsSources(Ardublockly.LANG);
  Ardublockly.updateLanguageText();
};
...
Ardublockly.injectLanguageJsSources = function(langKey) {
  ...
  // 先載入預設語系檔
  blocklyLangJsLoad = document.createElement('script');
  blocklyLangJsLoad.src = '../blockly_flag/msg/js/'+flagsBlockDefaultLang+'.js';
  head.appendChild(blocklyLangJsLoad);  

  blocklyLangJsLoad = document.createElement('script');
  blocklyLangJsLoad.src = '../blockly_flag/msg/js/' + langKey + '.js';
  head.appendChild(blocklyLangJsLoad);
};
...
Ardublockly.changeLanguage = function() {
  ...
  var changeLanguageHandler = function() {
  ...
  if (flagsBlockProjectIsChanged) {
    ...
    Ardublockly.alertMessage(
        Ardublockly.getLocalStr('changeLanguageReloadTitle'),
        Ardublockly.getLocalStr('changeLanguageReloadBody'),
        true, changeLanguageHandler);
  }
  else {

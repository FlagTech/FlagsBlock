...
long sounddata_length=0;
volatile long sample;
byte lastSample;
unsigned long lastTick;

void startPlayback(unsigned char const *data, long length)
{
  sounddata_data = data;
...
bool updatePlayback() {
  ...
  if(micros() - lastTick > 125) {
    if (sample >= sounddata_length) {
      ...
      analogWrite(speakerPin, pgm_read_byte(&sounddata_data[sample]) * 4);
      yield();
      ++sample;
      lastTick = micros();
    }
